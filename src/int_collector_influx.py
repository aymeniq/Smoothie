...
#         self.host_prefix = list(hosts)[0][0]

#         self.host_per_s = 0

#         self.switches = list(topology.get('switches'))
#         #print(self.switches)
#         s1 = self.switches[0]
#         for x in self.G.edges(s1):
#             if x[1][0] == self.host_prefix:
#                 self.host_per_s += 1
...
#             addr1 = params.get('addr1')
#             addr2 = params.get('addr2')
#             if params.get('bw')==None:
#                 self.G.add_edge(node1, node2, bw=10)
#             else:
#                 self.G.add_edge(node1, node2, bw=int(params.get('bw')))

#             if self.G[node1][node2]["bw"] > self.max_bw:
#                 self.max_bw = self.G[node1][node2]["bw"]

#             self.G[node1][node2][node1+"addr"] = addr1
#             self.G[node1][node2][node2+"addr"] = addr2


#         print("BW: {}".format(self.max_bw))
#         for n in self.G.nodes(data=True):
#             n[1]["q_load"] = 1
#             #print("link {} {}".format(node1, node2))
#             #print(params.get('bw'))

#     def set_weights(self):
#         self.max_bw = 0.0 #used to normalize link capacity between [0, 1]
#         node1=""
#         node2=""

#         for n in list(self.G.edges(data=True)):
#             if n[2]["bw"] > self.max_bw:
#                 self.max_bw = n[2]["bw"]
...
#         #generator = nx.all_simple_paths(self.G, source="h1", target="h2")
...
#         queue_load = 0
#         current_path = []
#         src_host = self.host_prefix+str(ipv6_to_id(report.flow_id["srcip"], self.host_per_s))
#         dst_host = self.host_prefix+str(ipv6_to_id(report.flow_id["dstip"], self.host_per_s))
...
#                 self.G.edges[src_node, dst_node][dst_node+"weight"] = (queue_load/queue_size) / (self.G.edges[src_node, dst_node]["bw"] / self.max_bw)
...
#         ips = []
#         for n in p[1:]:
#             if n[0] == self.host_prefix:
#                 ips.append(converttov6(mac_to_v4(self.G[src][n][n+"addr"])))
#             else:
#                 ips.append(mac_to_ipv6_linklocal(self.G[src][n][n+"addr"]))
#                 src = n
#         return ips

#     def export_path(self, p):
