...
import os
import time
from ipaddress import ip_interface, IPv6Network
...
from src.hostv6 import SRv6Host
...
def ipv62mac(ipv6):
    ...
    macParts[0] = "%02x".format(int(macParts[0], 16) ^ 2)
    del macParts[4]
    del macParts[3]
...
class NetworkAPI(Topo):
    ...
    def __init__(self, *args, **params):
        ...
        self.ipv4_net = IPv6Network('2001:1::/64')
        # Gateway static ARP
...
# Generic methods
    ...
    def startNetwork(self):
        ...
        create_int_collection_network(self.net.p4switches, influxdb="InfluxDB 172.18.0.3:8086")
...
# Assignment strategies
    def l2(self):
        ...
        **Assumes**
        ...
        ip_generator = IPv6Network('2001:1::/64').hosts()
        reserved_ips = {}
        assigned_ips = set()
        ...
        for node in self.nodes():
            ...
            if self.isHost(node):
                ...
                if self.check_host_valid_ip_from_name(node):
                    ...
                    host_ip = '2001:1::1:%d:%d' % (upper_byte, lower_byte)
        ...
        for node1, node2 in self.links():
            ...
            host_mac = ipv62mac(host_ip) % (0)
            direct_sw_mac = ipv62mac(host_ip) % (1)
        ...
        **Assumes**
        ...
        for node, info in self.nodes(withInfo=True):
            ...
            if self.isSwitch(node):
                ...
                net = '2001:1:%d:%d::0/64' % (upper_bytex, lower_bytex)
                sw_to_generator[node] = IPv6Network(str(net)).hosts()
                sw_to_id[node] = sw_id
            else:
        ...
        for node1, node2 in self.links():
            ...
            if self.check_host_valid_ip_from_name(host_name):
                ...
                assert host_num < 254
                host_ip = '2001:1::%d:%d:%d' % (upper_byte, lower_byte, host_num)
        ...
        for node1, node2 in self.links():
            ...
            host_gw = '2001:1::%d:%d:254' % (upper_byte, lower_byte)
            host_mac = ipv62mac(host_ip) % (0)
            direct_sw_mac = ipv62mac(host_ip) % (1)

            self.setIntfMac(host_name, direct_sw, host_mac)
            self.setIntfMac(direct_sw, host_name, direct_sw_mac)
            self.setIntfIp(host_name, direct_sw, host_ip + '/64')
            self.setIntfIp(direct_sw, host_name, host_gw + '/64')
        ...
        for node1, node2 in self.links():
            ...
            if self.isSwitch(node1) and self.isSwitch(node2):
                sw1_ip = '2001:1::%d:%d:1/64' % (sw_to_id[node1], sw_to_id[node2])
                sw2_ip = '2001:1::%d:%d:2/64' % (sw_to_id[node1], sw_to_id[node2])
        ...
        **Assumes**
        ...
        for node1, node2 in self.links():
            ...
            if self.check_host_valid_ip_from_name(host_name):
                host_num = int(host_name[1:])
                assert host_num < 254
                host_ip = '2001:1::%d:%d:2' % (sw_id, host_num)
                host_gw = '2001:1::%d:%d:1' % (sw_id, host_num)
                # We check if for some reason the ip was already given by the ip_generator.
                # This can only happen if the host naming is not <h_x>.
                # This should not be possible anymore since we reserve ips for h_x hosts.
                if host_ip in assigned_ips:
