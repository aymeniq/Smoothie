...
def create_int_collection_network(switches): #, influxdb
    ...
    #start_int_collector(conf, influxdb)
...
def start_int_collector(influxdb):
    ...
    collector_cmd = 'ip netns exec ns_int python3 ./src/int_collector_influx.py -i 6000 -H 192.168.0.1:8086 -d 1 -p 0 &' #> /dev/null
    print(collector_cmd)
...
def create_int_collector_link(bridge_name):
    ...
    #quietRunNs( 'ip -n ns_int  addr add %s/24 dev int_collector' % CONTROL_CPU_IP)
    ...
    quietRunNs('sudo ip -netns ns_int link set dev lo up')
    quietRunNs('sysctl net.ipv4.ip_forward')
    quietRunNs('net.ipv4.conf.all.forwarding')
    quietRunNs('sysctl net.ipv4.conf.int_collector.rp_filter=0')
    quietRunNs('sysctl net.ipv4.conf.all.rp_filter=0')
