...
void open_connections() {
  ...
  for (int i = 0; i < num_dest; i++) {
    ...
    if (getsockopt(sock, SOL_SOCKET, SO_RCVBUF, &len, &size) < 0) {
    ...
    if (getsockopt(sock, SOL_SOCKET, SO_SNDBUF, &len, &size) < 0) {
    ...
    size_t t1 =10000000;
    if (setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &t1, size) < 0) {
        printf("Unable to get sndbuf size. error: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_SNDBUF, &t1, size) < 0) {
        printf("Unable to get sndbuf size. error: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    ...
    servaddr.sin6_port = htons(dest_port[i]);
    inet_pton(AF_INET6, dest_addr[i], &servaddr.sin6_addr);
